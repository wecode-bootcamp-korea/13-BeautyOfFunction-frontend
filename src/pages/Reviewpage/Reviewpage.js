  ...
  changeName = (el, idx) => {
    ...
    if (idx === 0) {
      ...
      this.setState({ hairtype: index }, () => {
        fetch(
          `http://10.58.7.106:8000/review/get?product_id=${this.state.product_id}&offset=0&limit=5&hair_type=${this.state.hairtype}&hair_structure=${this.state.hairstructure}&scalp_moisture=${this.state.scalpmoisture}`
        )
          .then((res) => {
            return res.json();
          })
        ...
        <div className="productToggleContainer">
          ...
          <div className={list ? "listContainer" : "listContainer None"}>
            {titleList?.map((el, idx) => (
              <div onClick={() => this.changeTitle(el)}>{Object.keys(el)}</div>
            ))}
          </div>
        </div>
